...
import {getLatestPearlRaffle, updatePearlRaffle,  getBuyablePearlRaffle} from '../models/pearlRaffleModel';
...
import {getLatestShellRaffle, updateShellRaffle, getBuyableShellRaffle} from '../models/shellRaffleModel';
...
export const createPearlRaffleJob = () => {
    ...
    cron.schedule('0 0 * * 0', async () => {
        try {
            ...
            await updatePearlRaffle({id: latestPearlRaffle.id, indestructible: true});
...
export const createShellRaffleJob = () => {
    ...
    cron.schedule('0 0 * * 0', async () => {
        try {
            ...
            await updateShellRaffle({id: latestShellRaffle.id, indestructible: true});
    ...
    cron.schedule('0 0 * * 0', async () => {
        try {
            ...
            if(participants < pickPearlRaffle.min_participants){
                ...
                throw new CustomError(400, 'not enough participants');
            }

            const pearlRaffleArray = await getAllPearlRaffleIdArray(pickPearlRaffle.id);
            if(!pearlRaffleArray){
...
export const chooseShellRaffleWinnerJob = () => {
    ...
    cron.schedule('0 0 * * 0', async () => {
        try {
            ...
            const shellRaffleArray = await getAllShellRaffleIdArray(pickShellRaffle.id);
...
const refundPearlRaffleJob = async() => {
    try {
        ...
        await updatePearlRaffle({id: pickedPearlRaffle.id, done: true});
        console.log('환불 종료')
    } catch (error) {
        console.error(error);
        throw error;
...
const refundShellRaffleJob = async() => {
    try {
        ...
        raffle_array.map(async (item) => {
            //1개 래플 로그 객체를 이용해서 유저 아이디 뽑아내고, 특정 reason 이용해서 해당 트랜잭션 가져오기.
        ...
        await updateShellRaffle({id: pickedShellRaffle.id, done: true});
        console.log('환불 종료')
    } catch (error) {
        console.error(error);
        throw error;
